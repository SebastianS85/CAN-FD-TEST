...
// ============================================================================
// TRYBY PRACY (USTAWIANE RĘCZNIE)
// ============================================================================
#define APP_MODE_ENABLE_TCP 1
#define APP_MODE_ENABLE_SD  0

// ============================================================================
// TCP NETWORK CONFIGURATION (ESP32 jest Serwerem, PC jest Klientem)
// ============================================================================
#define TX_PORT 3333    
#define RX_PORT 3334    

// ============================================================================
// CAN, I2C & SD CONFIGURATION
// ============================================================================
...
// ============================================================================
// DATA STRUCTURES (Dokładnie 74 bajty)
// ============================================================================
...
// ============================================================================
// MASSIVE SINGLE PSRAM RINGBUFFER (6MB)
// ============================================================================
...
static volatile bool g_pc_connected = false; // Flaga blokująca bufor do momentu połączenia z PC
...
static void app_can_rx_handler(uint8_t node_id, const twai_frame_t *rx_frame, void *user_ctx) {
    ...
    if (rx_frame->header.ide) frame.id |= 0x80000000U;
    ...
    // Zapisujemy do bufora TYLKO wtedy, gdy komputer jest faktycznie połączony!
    ...
    if (node_id == 1) g_rx_frames_node1++;
    else if (node_id == 2) g_rx_frames_node2++;
...
void tcp_sender_task(void *pvParameters) {
    ...
    ESP_LOGI("TCP_SERVER", "Serwer TCP gotowy na porcie %d. Czekam na PC...", TX_PORT);
    ...
    while (1) {
        ...
        // ESP32 czeka tutaj, aż Python się podłączy
        ...
        ESP_LOGI("TCP_SERVER", "KOMPUTER PODŁĄCZONY! Otwieram strumień...");
        g_pc_connected = true; // Odblokowujemy bufor CAN
        ...
        while (1) {
            ...
            if (data != NULL) {
                ...
                if (frame_count >= FRAMES_PER_PACKET) {
                    ...
                    if (err < 0) {
                        ESP_LOGE("TCP_SERVER", "Utrata połączenia z PC!");
                        break; 
                    }
        ...
        g_pc_connected = false; // Blokujemy bufor po rozłączeniu
        close(sock);
        ESP_LOGW("TCP_SERVER", "Rozłączono. Powrót do nasłuchu...");
    }
}
        ...
        while (1) {
            ...
            if (rx_bytes == sizeof(cmd)) { 
                ...
                if (!target_node->recovery_in_progress) {
                    ...
                    if (cmd.node_id == 1) g_tx_frames_node1++;
...
void diagnostics_task(void *pvParameters) {
    while (1) {
        ...
        printf("[AUDYT TCP] CAN(RX): %lu | Weszlo: %lu | Wyszlo: %lu | ZALEGA w PSRAM: %lu | TCP Wyslane: %lu\n",
...
void sd_writer_task(void *pvParameters) {
    ...
    while (1) {
        if (g_stop_logging) {
            if (current_bytes > 0) sdcard_service_append_bin_block(log_filename, write_buffer, current_bytes);
            sdcard_service_sync();    
...
void app_main(void) {
    ...
    if (APP_MODE_ENABLE_SD) {
        sdcard_service_config_t sd_cfg = {
            .pin_mosi = PIN_NUM_MOSI, .pin_miso = PIN_NUM_MISO, .pin_sclk = PIN_NUM_CLK,
            .pin_cs = PIN_NUM_CS, .host_id = SPI2_HOST, .format_if_mount_failed = true,
    ...
    // Ustawienie prędkości automotive: 500 kbps arbitraż / 2 Mbps dane
    ESP_ERROR_CHECK(twai_mgr_init_custom_node(&node1, TX_PIN_NODE_1, RX_PIN_NODE_1, 
                                               TWAI_BUS_MODE_FD, 500000, 2000000, 
    ...
    gpio_config_t btn_conf = {
        .intr_type = GPIO_INTR_NEGEDGE, .pin_bit_mask = (1ULL << EJECT_BTN_PIN),
